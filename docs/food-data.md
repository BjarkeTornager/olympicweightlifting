...
The current web and native clients identify food-tag support with `X-Food-Tags-Version: 1` on account requests. Older web clients receive compatible snapshots without the new fields on journal reads, saves, conflicts and Coach save/undo responses; the stored tags remain intact. All journal HTTP writes preserve omitted classifications for uniquely matched item names/portions. Ambiguous matches are rejected for review. Explicit empty arrays clear tags. Server-owned Coach undo restores the exact previous snapshot, including missing metadata. Tag-aware manual undo explicitly clears tags absent from its prior state; unversioned cached undo copies cannot silently clear tags. Pre-release mutation hashes remain valid because optional classification has no schema default.
