...
Olympic lifting also has curated coaching/nutrition guidance and experimental video-frame review. See [sources, private media handling and evaluation limits](lifting-sources-video-2026-09-10.md). Private saved video uploads and background FFmpeg/OpenCV processing are now available; see [video analysis operations and limits](video-analysis-2026-09-11.md). The alternative on-device frame review keeps the source local.
...
Agent messages are saved in `agent_turns`. `agent_proposals` holds a 24-hour snapshot of a proposed change, its owner, original revision and stable save/undo IDs. The `log_entry` tool saves reported meals, check-ins, cardio, performed training and requested corrections without a confirmation button. Its journal write, saved receipt, Undo snapshot and completed turn commit in one transaction. The separate `prepare_change` tool remains an unsaved review for previews, deletions, targets, personal bests, memories and plans. Its confirmation button still commits atomically. A later journal revision blocks a stale proposal or undo. Repeated identical confirmations are idempotent. A cancelled or failed run before commit writes no entries. A connection loss after commit can still mean a successful save: `GET /api/agent?turnId=<uuid>` retrieves only the authenticated account’s durable result. Retrying the same message ID returns that receipt rather than writing again; failed transactions can retry with the same ID and latest revision. The web client advertises `X-Coach-Logging-Version: 1`, refreshes journal data after a saved receipt and recovers lost responses before restoring the draft. Older clients retain reviewed logging until upgraded. Existing pending proposals are never automatically applied during migration or reconnect.
