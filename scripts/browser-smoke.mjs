...
try {
  ...
  priorWorkout.exercises[0].strongSets = true;
  priorWorkout.exercises[0].sets.forEach(set => { set.weight = "45"; set.reps = "1"; set.result = "success"; set.logged = true; set.rpe = "7"; });
  ...
  await until("document.querySelector('.is-active') !== null");
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "45");
  await select("#workout-recovery", "good");
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "47.5");
  ...
  assert.ok(await evaluate("document.querySelector('.is-active .progression-body').innerText.includes('recovery is good')"));
  await tap('.is-active .progression-note > summary');
  ...
  await tap('.is-active [data-action="strong-sets"]');
  await tap('[data-action="complete-exercise"]');
  await tap('.workout-topbar [data-action="finish-workout"]');
  await until("location.hash === '#history'");
  ...
  await reload(".is-active");
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "50");
  pass("Automatic 45 → 47.5 → 50 kg progression, recovery holds, pending-set updates, explicit completion, same-day guard and persisted targets");
  await route("#data", "#data-title");
  await importFile(downloadDirectory + "/" + backupFile);
  ...
  assert.ok(await evaluate("(async()=> (await caches.keys()).includes('lift-journal-shell-v4'))()"));
