...
try {
  ...
  await tap(first + ' [data-action="adjust-weight"][data-delta="2.5"]');
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "47.5");
  await tap(first + ' [data-action="adjust-weight"][data-delta="-2.5"]');
  ...
  await route("#dashboard", "#dashboard-title");
  await tap('[data-action="select-day"][data-day-id="monday"]');
  assert.equal(await evaluate("document.querySelector('[data-program-targets=monday] [data-program-exercise=snatch]').dataset.targetWeight"), "47.5");
  await capture("program-next-loads-home-390");
  await route("#workout", "#workout-title");
  assert.equal(await evaluate("document.querySelector('[data-program-day=monday] [data-program-exercise=snatch]').dataset.targetWeight"), "47.5");
  ...
  assert.ok(await evaluate("document.querySelector('.is-active .exercise-summary').textContent.includes('47.5 kg')"));
  ...
  assert.ok(completedProgression.sessions[1].exercises[0].sets.every(set => set.weight === "47.5"));
  ...
  await route("#workout", "#workout-title");
  assert.equal(await evaluate("document.querySelector('[data-program-day=monday] [data-program-exercise=snatch]').dataset.targetWeight"), "50");
  assert.ok(await evaluate("document.querySelector('[data-program-day=monday] .program-repeat-note').textContent.includes('repeat today')"));
  await tap('[data-action="start-day"][data-day-id="monday"]');
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "47.5", "Same-day repeats must not compound increases");
  ...
  await reload(".is-active");
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "50");
  pass("Program previews and automatic 45 → 47.5 → 50 kg loads without opt-in, recovery holds, carry-forward, same-day guard and persistence");
  ...
  await route("#workout", "#workout-title");
  assert.equal(await evaluate("document.querySelector('[data-program-day=monday] [data-program-exercise=snatch]').dataset.targetWeight"), "57.5");
  await tap('[data-action="start-day"][data-day-id="monday"]');
  assert.equal((await saved()).activeWorkout.exercises[0].sets[0].weight, "57.5");
