...
Find up to three complete attempts across the full video. Separate attempts only when the lifter actually starts another repetition. Never split the clean and the later jerk into separate attempts; include the rack pause, recovery and the overhead action together. startFrame/endFrame enclose the full attempt using printed frame labels. Each evidence phase belongs inside its attempt's range. Ranges must be chronological and non-overlapping. If more than three repetitions or boundaries cannot be established, return one limited attempt explaining that a shorter clip is needed. Do not invent a lift when no lifting is visible. Each attempt's evidence obeys these definitions:` +
    original
      .slice(shapeEnd)
      .replace(
